...
SQUAD_PERMISSIONS = {
    "auth_squad": {
        "ALLOWED_TYPES": ["auth", "authentication", "security", "jwt", "login"],
        "FORBIDDEN_KEYWORDS": ["css", "jsx", "database migration", "raw sql index"]
    },
    "db_squad": {
        "ALLOWED_TYPES": ["database", "schema", "migration", "model", "sql"],
        "FORBIDDEN_KEYWORDS": ["react component", "jwt token", "http router"]
    },
    "api_squad": {
        "ALLOWED_TYPES": ["api", "route", "endpoint", "controller", "rest"],
        "FORBIDDEN_KEYWORDS": ["database migration", "css styling"]
    },
    "ui_squad": {
        "ALLOWED_TYPES": ["ui", "frontend", "component", "layout", "view"],
        "FORBIDDEN_KEYWORDS": ["alembic migration", "bcrypt hashing"]
    }
}
...
class AuthSquadAgent:
    ...
    def execute_auth_task(self, task: dict, global_context: str = "") -> dict:
        ...
        # Law 20 Enforcement: Check squad boundary
        if any(kw in task_title for kw in SQUAD_PERMISSIONS["auth_squad"]["FORBIDDEN_KEYWORDS"]):
            raise PermissionError(f"Law 20 Violation: AuthSquadAgent received out-of-scope task: '{task.get('title')}'")
...
class DatabaseSquadAgent:
    ...
    def execute_db_task(self, task: dict, global_context: str = "") -> dict:
        ...
        # Law 20 Enforcement: Check squad boundary
        if any(kw in task_title for kw in SQUAD_PERMISSIONS["db_squad"]["FORBIDDEN_KEYWORDS"]):
            raise PermissionError(f"Law 20 Violation: DatabaseSquadAgent received out-of-scope task: '{task.get('title')}'")
