...
def test_extract_target_keys():
    breaches = ["Missing mandatory schema key: 'metadata'", "Task 2 missing 'id'."]
    keys = SurgicalRefinerEngine.extract_target_keys(breaches)
    assert "metadata" in keys
    assert "id" in keys


def test_generate_surgical_instructions():
    breaches = ["Task 1 has invalid type 'xyz'."]
    feedback = SurgicalRefinerEngine.generate_surgical_instructions(breaches, ["type"])
...
def test_generate_refinement_feedback_pipeline():
    ...
    res = generate_refinement_feedback(breaches)
